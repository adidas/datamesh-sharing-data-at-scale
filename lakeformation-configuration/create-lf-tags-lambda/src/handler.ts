  ...
  public async execute(input: CloudFormationCustomResourceEvent, context: Context): Promise<void> {
    try {
      ...
      if ((input as any).PhysicalResourceId === RESOURCE_NOT_CREATED) {
        ...
        const physicalResourceId = await this.domain.createLFTag(input);
        ...
        const physicalResourceId = await this.domain.updateLFTag(input);
        ...
        const physicalResourceId = await this.domain.deleteLFTag(input);
