...
describe('# GetDataProductS3FilesChain Construct', () => {
  ...
  it('Should create EvaluateExpressions lambda invoke successfully', (done) => {
    ...
    expect(EvaluateExpression).toHaveBeenNthCalledWith(1, construct, 'Get Bucket S3 URI', {
      // eslint-disable-next-line no-template-curly-in-string
      expression: '`s3://${ $.dataProductProducerInfoObject.bucket-name }`',
      resultPath: '$.bucketS3Uri'
    });
    expect(EvaluateExpression).toHaveBeenNthCalledWith(2, construct, 'Get Bucket S3 ARN', {
      // eslint-disable-next-line no-template-curly-in-string
