...
export type GrantAccessToProducerLambdaProps = {
...
export class GrantAccessToProducerLambda extends Construct {
  /* AWS resources attached to this construct */
  public readonly lambda: LambdaFunction;

  public constructor(scope: Construct, id: string, props: GrantAccessToProducerLambdaProps) {
    super(scope, id);
  ...
  private setupLambda(scope: Construct, {
  ...
  }: GrantAccessToProducerLambdaProps): LambdaFunction {
    const {
      Logging: configLogging, Lambda: configLambda
    } = this.loadConfig(deploymentEnvironment);

    const lambdaFunctionId = 'GrantAccessToProducer';
    ...
    const lambda = new LambdaFunction(scope, lambdaFunctionId, {
      ...
      code: Code.fromAsset('./producer-journey/grant-access-to-producer-lambda/dist'),
