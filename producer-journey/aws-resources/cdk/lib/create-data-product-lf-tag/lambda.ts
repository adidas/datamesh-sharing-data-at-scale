...
export type CreateDataProductLFTagLambdaProps = {
...
export class CreateDataProductLFTagLambda extends Construct {
  /* AWS resources attached to this construct */
  public readonly lambda: LambdaFunction;

  public constructor(scope: Construct, id: string, props: CreateDataProductLFTagLambdaProps) {
    super(scope, id);
  ...
  private setupLambda(scope: Construct, {
  ...
  }: CreateDataProductLFTagLambdaProps): LambdaFunction {
    const {
      Logging: configLogging, Lambda: configLambda
    } = this.loadConfig(deploymentEnvironment);

    const lambdaFunctionId = 'CreateDataProductLFTag';
    ...
    const lambda = new LambdaFunction(scope, lambdaFunctionId, {
      ...
      code: Code.fromAsset('./producer-journey/create-data-product-lf-tag-lambda/dist'),
