...
export type CreateGlueDatabasesLambdaProps = {
...
export class CreateGlueDatabasesLambda extends Construct {
  /* AWS resources attached to this construct */
  public readonly lambda: LambdaFunction;

  public constructor(scope: Construct, id: string, props: CreateGlueDatabasesLambdaProps) {
    super(scope, id);
  ...
  private setupLambda(scope: Construct, {
  ...
  }: CreateGlueDatabasesLambdaProps): LambdaFunction {
    const {
      Logging: configLogging, Lambda: configLambda
    } = this.loadConfig(deploymentEnvironment);

    const lambdaFunctionId = 'CreateGlueDatabase';
    ...
    const lambda = new LambdaFunction(scope, lambdaFunctionId, {
      ...
      code: Code.fromAsset('./producer-journey/create-glue-database-lambda/dist'),
      handler: 'index.createGlueDatabaseHandler',
      description: 'Creates a glue database for IAM or LF producers from AWS',
