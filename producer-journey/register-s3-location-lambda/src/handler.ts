...
export class Handler {
  ...
  public async execute(input: HandlerInput): Promise<void> {
    try {
      ...
      const { bucketS3Arn, dataProductProducerInfoObject } = input;

      await this.domain.execute(bucketS3Arn, dataProductProducerInfoObject['producer-account-id']);
